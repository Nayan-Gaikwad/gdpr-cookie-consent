...
(function( $ ) {
	...
	var GDPR = {
		...
		set:function(args) {
			...
			window.addEventListener("load", function() {
				for(var id=0;id<gdpr_select_pages.length;id++)
				{
					...
					if (document.body.classList.contains('page-id-' + pageToHideBanner)) {
						if(GDPR.settings.cookie_usage_for == 'gdpr' || GDPR.settings.cookie_usage_for == 'eprivacy' || GDPR.settings.cookie_usage_for == 'both' || GDPR.settings.cookie_usage_for == 'lgpd'){
			...
			if ( this.settings.cookie_usage_for == 'gdpr' || this.settings.cookie_usage_for == 'eprivacy' || this.settings.cookie_usage_for == 'both' || this.settings.cookie_usage_for == 'lgpd' ) {	
		...
		consent_renew_method: function ( ) {
			...
			for (var i = 0; i < gdpr_consent_renew.length; i++) {
				if (gdpr_consent_renew[i].ip_value == gdpr_user_ip && gdpr_consent_renew[i].consent_value == "true") {
			...
			//ajax call to send the values back to callback action
			var data = {
				action: 'gdpr_renew_consent_bar',
				security: log_obj.consent_renew_nonce,
				arrayValue: gdpr_consent_renew,
			};
			$.ajax(
				{
					type: 'post',
							...
							if ('both' == cookie_for) {
								if (GDPR_Cookie.exists( GDPR_ACCEPT_COOKIE_NAME ) && GDPR_Cookie.exists( GDPR_CCPA_COOKIE_NAME )) {
									...
									if ( GDPR.settings.auto_hide ) {
										var banner_delay = GDPR.settings.auto_banner_initialize ? (parseInt(GDPR.settings.auto_hide_delay)+parseInt(GDPR.settings.auto_banner_initialize_delay) ): GDPR.settings.auto_hide_delay ;
										setTimeout(
											function(){
												GDPR.accept_close();
											},
		...
		attachEvents:function() {
			...
			jQuery( '.gdpr_messagebar_detail input' ).each(
				function(){
					...
					console.log("nayanaa")
					console.log(gdpr_user_preference_val)
					GDPR_Cookie.set( 'wpl_user_preference',gdpr_user_preference_val,GDPR_ACCEPT_COOKIE_EXPIRE );
				}
			);
			jQuery( document ).on(
		...
		configBar:function() {
			...
			if (this.settings.cookie_bar_as == 'widget') {
				...
				} else if ( this.settings.notify_position_horizontal == 'top_right' ) {
			...
			if ( this.settings.cookie_usage_for == 'gdpr' ) {
				if ( ! GDPR_Cookie.exists( GDPR_ACCEPT_COOKIE_NAME )) {
					...
					if ( this.settings.auto_hide ) {
						var banner_delay = this.settings.auto_banner_initialize ? (parseInt(this.settings.auto_hide_delay) +parseInt(this.settings.auto_banner_initialize_delay)) : this.settings.auto_hide_delay ;
						setTimeout(
							function(){
								GDPR.accept_close();
								GDPR.logConsent( 'accept' );
				...
				if (GDPR_Cookie.exists( GDPR_ACCEPT_COOKIE_NAME ) && GDPR_Cookie.exists( GDPR_CCPA_COOKIE_NAME )) {
					...
					if ( this.settings.auto_hide ) {
						var banner_delay = this.settings.auto_banner_initialize ? (parseInt(this.settings.auto_hide_delay) + parseInt(his.settings.auto_banner_initialize_delay)) : this.settings.auto_hide_delay ;
						setTimeout(
							function(){
								GDPR.accept_close();
								GDPR.logConsent( 'accept' );
		...
		displayHeader:function(gdpr_flag, ccpa_flag,lgpd_flag,force_display_bar, force_display_show_again) {
			...
			if (this.settings.cookie_usage_for == 'gdpr'||this.settings.cookie_usage_for == 'lgpd' || this.settings.cookie_usage_for == 'eprivacy' || this.settings.cookie_usage_for == 'both') {
	...
	$( document ).ready(
		function() {
			...
			$( ".gdpr_messagebar_detail.layout-default  .category-group").css( 'background-color', background_obj.background );
			$( ".gdpr_messagebar_detail.layout-default  .category-group.outer").css( 'border-left', '1px solid '+GDPR.settings.button_accept_button_color );
			$( ".gdpr_messagebar_detail.layout-default.dark_row  .category-group.outer").css( 'border-left', '1px solid #73DBC0' );
			...
			$( ".gdpr-iab-navbar-item" ).click(
				function() {
					...
					switch(this.id) {
						case "gdprIABTabCategory" : $( ".cat").css( 'display', 'block')
							break;
						case "gdprIABTabFeatures" : $( ".feature-group").css( 'display', 'block')
							break;
						case "gdprIABTabVendors" : $( ".vendor-group").css( 'display', 'block')
							break;
					}
			...
			$( ".gdpr-category-toggle.inner-gdpr-column" ).click(
				function() {
			...
			$( ".gdpr-default-category-toggle.inner-gdpr-column" ).click(
				function() {
					...
					if ( ! $( this ).children( ".inner-gdpr-columns" ).hasClass( "active-group" ) ) {
						...
						$( ".inner-gdpr-columns" ).css( 'background-color', background_obj.background );
						$( this ).children( ".inner-gdpr-columns" ).addClass( "active-group" )
						$( this ).children( ".inner-gdpr-columns" ).css( 'background-color', GDPR.settings.button_accept_button_color )
					}
					if ( $( this ).siblings( ".inner-description-container" ).hasClass( "hide" ) ) {
						$( ".inner-description-container" ).addClass( "hide" )
						$( this ).siblings( ".inner-description-container" ).removeClass( "hide" )
